...
static NSArray<ZHNACompiledRule *> *ZHNACompiledRules(void) {
    ...
    dispatch_once(&once, ^{
        ...
        NSArray *raw = @[
            ...
            @[@"^https?://[^/]*\\.zhihu\\.com/api/v4/(answers|questions)/\\d+/related-readings", @(ZHNAActionEmptyJSON), @"相关阅读推广", ZHNAKeyDetail],
            @[@"^https?://[^/]*\\.zhihu\\.com/api/v4/(articles|answers)/\\d+/recommendations?", @(ZHNAActionEmptyJSON), @"详情页推荐位", ZHNAKeyDetail],
